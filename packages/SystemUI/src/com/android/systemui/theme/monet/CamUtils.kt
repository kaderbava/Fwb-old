...
public object CamUtils {
    ...
    public fun intFromLstar(lstar: Double): Int {
        if (lstar < 1.0) {
            return -16777216
        }
        if (lstar > 99.0) {
    ...
    public fun lstarFromInt(argb: Int): Double {
    ...
    public fun yFromInt(argb: Int): Double {
    ...
    public fun xyzFromInt(argb: Int): DoubleArray {
        val r = linearized(Color.red(argb))
        val g = linearized(Color.green(argb))
        val b = linearized(Color.blue(argb))
        val matrix = SRGB_TO_XYZ
